...
def process_trade_file(zipped_trade_filename, parent_dir=None, temp_dir=None):
    ...
    with gzip.open(full_output_filename, 'rb') as f_in:
        ...
        q.query(qconnection.MessageType.SYNC, f'trades:("SDIIIIFI";enlist "|") 0: `:{extracted_file};')
        q.query(qconnection.MessageType.SYNC,
                'update sym:ISIN, time: (1000*TimeMM) + Date + `second$(60*60*TimeSec div 10000) + (60 * (TimeSec mod 10000) div 100) + (TimeSec mod 100) from `trades;')
        q.query(qconnection.MessageType.SYNC,
                'delete from `trades where Price=0;delete ISIN, TimeMM, Date, TimeSec, MarketTime from `trades;')
        q.query(qconnection.MessageType.SYNC, f'trades:`sym`time`Price`Qty`Volume xcols tab;')
        q.query(qconnection.MessageType.SYNC, f'.Q.dpft[`:I:/testkdb/trades/;{date_string};`sym;`trades];')
        
        # trades:("SDIIIIFI";enlist "|") 0: `:I:/beetroot/csv_data_from_py/20170505_EUX_MKtrade.csv;
        # update sym:ISIN, time: (1000*TimeMM) + Date + `second$(60*60*TimeSec div 10000) + (60 * (TimeSec mod 10000) div 100) + (TimeSec mod 100) from `trades;
        # delete from `trades where Price=0;
        # delete ISIN, TimeMM, Date, TimeSec, MarketTime from `trades;
        # trades:`sym`time`Price`Qty`Volume xcols tab;
        # .Q.dpft[`:I:/testkdb/trades/;2017.05.05;`sym;`trades];
        # delete trades from `.;
...
def main():
    ...
    q.close()
    logging.info(
        f"Closed Q connection {q}." + 'IPC version: %s. Is connected: %s' % (q.protocol_version, q.is_connected()))
...
#
# meta trades
# select from trades where Qty >10
